...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['submit_gram_panchayat'])) {
        ...
        $gram_panchayat_name = mysqli_real_escape_string($conn, $gram_panchayat_name);
        ...
        // Check if gram_panchayat_name already exists for the selected district, vidhansabha, vikaskhand, and sector
        $check_query = "SELECT * FROM gram_panchayat_master WHERE gram_panchayat_name = '$gram_panchayat_name' AND district_id = '$district_id' AND vidhansabha_id = '$vidhansabha_id' AND vikaskhand_id = '$vikaskhand_id' AND sector_id = '$sector_id'";
        $check_result = mysqli_query($conn, $check_query);

        if (mysqli_num_rows($check_result) > 0) {
            // Gram Panchayat name already exists
            echo "<script>alert('Error: Gram Panchayat already exists!');</script>";
        } else {
            // Gram Panchayat name does not exist, proceed with insertion
            $sql = "INSERT INTO gram_panchayat_master (gram_panchayat_name, district_id, vidhansabha_id, vikaskhand_id, sector_id) VALUES ('$gram_panchayat_name', '$district_id', '$vidhansabha_id', '$vikaskhand_id', '$sector_id')";
            if (mysqli_query($conn, $sql)) {
                echo "<script>alert('Gram Panchayat Added Successfully');</script>";
            } else {
    ...
    <form method="post">
        <div class="row text-center align-items-center">
            ...
            <div class="col-lg-4 text-center mb-3">
                <input type="text" name="gram_panchayat_name" class="form-control border-success" placeholder="ग्राम पंचायत का नाम" required>
            </div>
